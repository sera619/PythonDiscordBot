        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise YTDLError('Konnte keinen Eintrag für folgende URL finden: `{}`'.format(webpage_url))
...
class Music(commands.Cog):
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Vote-Skip gestartet, Stimmen: **{}/3**'.format(total_votes))
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send('Ein Fehler ist augetreten beim bearbeiten der Anfrage: {}'.format(str(e)))
            else:
                song = Song(source)
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Du bist mit keinem Voice-Channel verbunden. Bitte joine einem Voice-Channel um den MusikPlayer zu benutzen.')
